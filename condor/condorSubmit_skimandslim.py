...
parser.add_option ('-t',  dest='trigfile', type='string', default = 'skim-slim-trigs.txt', help="File with list of triggers to require")

options, args = parser.parse_args()

# Prepare the list of files to transfer

filestoTransfer = [environ["CMSSW_BASE"] + "/src/Exploration/skim_and_slim_exe", 
                   environ["CMSSW_BASE"] + "/src/Exploration/skim-slim-trigs-ele.txt",
                   environ["CMSSW_BASE"] + "/src/Exploration/skim-slim-trigs-mu.txt",
                   environ["CMSSW_BASE"] + "/src/Exploration/skim-slim-save-vars.txt"
                   ]
...
Executable = run_skim_and_slim_exe_condor.tcsh
    ...
    for s, n in sc.sampleList(ds):
        ...
        if not f == None:
            ...
            for startFileNum in xrange(0, count, nFilesPerJob):
                fileParts.append("transfer_output_remaps = \"skimandslim_%s_%s.root = output-files/%s/skimandslim_%s_%s.root\"\n" % (n, startFileNum, ds, n, startFileNum))
                fileParts.append("Arguments = %s %i %i %s skim  lepton_skim  false  %s\n"%(n, nFilesPerJob, startFileNum, s, options.trigfile))
                fileParts.append("Output = log-files/skimandslim_%s_%i.stdout\n"%(n, startFileNum))
                fileParts.append("Error = log-files/skimandslim_%s_%i.stderr\n"%(n, startFileNum))
                fileParts.append("Log = log-files/skimandslim_%s_%i.log\n"%(n, startFileNum))
