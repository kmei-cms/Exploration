...
#ifndef NtupleClassMicroNtuple_h
#define NtupleClassMicroNtuple_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   NtupleClassMicroNtuple(TTree *tree=0, TTree* output_tree_arg=0, bool do_skim_arg=false );
   virtual ~NtupleClassMicroNtuple();
...
#ifdef NtupleClassMicroNtuple_cxx
NtupleClassMicroNtuple::NtupleClassMicroNtuple(TTree *tree, TTree* output_tree_arg, bool do_skim_arg ) : fChain(0) 
{
  do_skim = do_skim_arg ; 
...
NtupleClassMicroNtuple::~NtupleClassMicroNtuple()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t NtupleClassMicroNtuple::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t NtupleClassMicroNtuple::LoadTree(Long64_t entry)
{
...
void NtupleClassMicroNtuple::Init(TTree *tree)
{
...
Bool_t NtupleClassMicroNtuple::Notify()
{
...
void NtupleClassMicroNtuple::Show(Long64_t entry)
{
...
Int_t NtupleClassMicroNtuple::Cut(Long64_t entry)
{
...
#endif // #ifdef NtupleClassMicroNtuple_cxx
